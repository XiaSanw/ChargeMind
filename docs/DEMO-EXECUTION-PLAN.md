...
### LLM Prompt 模板（固定框架 + 自由发挥 + 数据驱动洞察）
...
【输出要求】
请按以下三层结构输出分析报告，使用 Markdown 格式。

⚠️ 重要：三层标题必须保留，但每层内部的具体分析重点、子项顺序、详略程度由你根据数据自由判断。不要机械填充模板，要像专家一样根据数据差异灵活调整分析重心。
...
**开头必须用一句话直击痛点：**
"你的场站预计年收益 X 万元。同类场站中，最高 Y 万元，最低 Z 万元，差距 W 万元。"

然后展开：
- 估算全年充电量和全年收益（基于算法预测）
- 列出关键运营指标（利用率、日均充电量、高峰时段）
- 与相似场站对比，定位当前水平

---

## 第二部分：潜力空间测算（RAG 数据驱动洞察）

**核心任务：把相似场站按运营表现分为两组进行对比**

- **标杆场站**（运营表现优异）：共同特征是什么？（提炼 2-3 个）
- **常规场站**（运营表现一般）：共同特征是什么？（提炼 2-3 个）
- 用户场站目前更接近哪一组？
- **从标杆到场站，收益差距是多少？**（用具体数字说明）

**异常值洞察：**
如果对标数据中有异常值（如功率异常、利用率异常），不要忽略，分析它背后可能隐藏的需求或风险。
...
- 初版允许基于描述性输入做合理推断
- 三层结构固定，内容自由发挥
- 收益差距必须量化（不要说"有较大提升空间"，要说"预计可提升 X 万元/年"）
```
